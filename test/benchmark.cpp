...
static void bench_trace(benchmark::State &state) {
  ...
  scene scene;
  scene.addObject(std::make_unique<Sphere>(Vector3f{0, 0, 0}, 1, Color::RED));
  scene.addObject(
      std::make_unique<Sphere>(Vector3f{0, 1, 0}, 0.5, Color::GREEN));
  scene.addObject(std::make_unique<Sphere>(Vector3f{0, -1, 0}, 1, Color::BLUE));
  scene.addLight(std::make_unique<PointLight>(Vector3f{2, 2, 2}, Color::WHITE));
